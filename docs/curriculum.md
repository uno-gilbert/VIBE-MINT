...
| **15:00 ~ 16:30** | **NFT 컨트랙트 작성 및 멀티체인 배포** | · Remix에서 컴파일·최적화 후 **Ethereum Sepolia** 테스트넷 배포 (MetaMask 연동)<br><br>· Faucet으로 테스트 ETH 수령 → 배포 → **테스트 mint** 성공까지<br><br>· Etherscan Sepolia에서 컨트랙트·트랜잭션 확인<br><br>· **멀티체인 개념 소개**: L2(Base, Arbitrum 등)와 Sepolia L1의 역할 비교 — *당일 hands-on은 Sepolia 중심*<br><br>· **(선택)** IPFS(Pinata)로 metadata 업로드·`setBaseURI` — [06-ipfs-metadata.md](student/06-ipfs-metadata.md)<br><br>· 참고: [04-deploy-sepolia.md](student/04-deploy-sepolia.md) |
